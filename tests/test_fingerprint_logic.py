...
def test_fingerprint_name_reversal():
...
def test_fingerprint_name_reversal_multiple_words():
    p1: Participant = {
...
def test_fingerprint_no_match_different_club():
    p1: Participant = {
        "name": "Magnus Blåudd",
        "club": "Club A",
        "class_name": "H",
...
def test_generate_fingerprints_with_known_hashes():
    participants = [
    ...
    import hashlib
    known_hash = hashlib.sha256("magnus blåudd|ok skogsstjärnan".encode("utf-8")).hexdigest()
    
    fingerprints = Fingerprinter.generate_fingerprints(participants, known_hashes={known_hash})
