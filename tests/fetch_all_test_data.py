...
def fetch_and_save(scraper, url, filename):
...
def main():
    ...
    for event in events:
        ...
        # Determine if single or multi based on content (simple heuristic or just save as _main for now)
        # The user requested standardized names: COUNTRY_ID_TYPE.html
        # We'll use _main.html for the event page, and _entries, _start_list, _result_list for lists.
        ...
        for a in soup.find_all('a', href=True):
            ...
            if '/Events/Show/' in href and href != event['url']:
                match = re.search(r'/Events/Show/(\d+)', href)
        ...
        if race_links:
            ...
            for i, (race_id, race_url) in enumerate(race_links, 1):
                ...
                if race_html:
                    ...
                    for list_type, suffix in [('StartList', 'start_list'), ('ResultList', 'result_list')]:
                        list_url = f"{event['base_url']}/Events/{list_type}?eventId={race_id}&groupBy=EventClass"
                        list_filename = f"{event['country']}_{event['id']}_race{i}_{suffix}.html"
        ...
        # Fetch race-specific lists for SWE/NOR multi-race events
            ...
            for i, race_id in enumerate(sorted(race_ids), 1):
                for list_type, suffix in [('StartList', 'start_list'), ('ResultList', 'result_list')]:
                    list_url = f"{event['base_url']}/Events/{list_type}?eventId={event['id']}&eventRaceId={race_id}&groupBy=EventClass"
                    list_filename = f"{event['country']}_{event['id']}_race{i}_{suffix}.html"
                    fetch_and_save(scraper, list_url, list_filename)
                    time.sleep(1)
        ...
        # Fetch single-event lists (for single-race events or event-level lists)
            ...
            for list_type, suffix in [('Entries', 'entries'), ('StartList', 'start_list'), ('ResultList', 'result_list')]:
                list_href = get_list_url(soup, list_type)
                if list_href:
                    full_url = f"{event['base_url']}{list_href}" if list_href.startswith('/') else list_href
