...
1.  **Umbrella Index**: `mtbo_events.json` - Contains metadata, partition links, and global source statistics.
...
1.  **Syncs Data**: Downloads the latest `mtbo_events.json` and all relevant partition files.
...
- Fetches `mtbo_events.json` from the scraper repository.
- Parses `mtbo_events.json` to identify active partitions in the `partitions` object.
...
- The schema contains a `oneOf` rule: it validates both the `UmbrellaIndex` (mtbo_events.json) and the `EventListWrapper` (partition files).
- **Example Validation Logic**:
  ```python
  from jsonschema import validate
  with open("schema.json") as s, open("mtbo_events.json") as d:
...
- **Global Sources**: In `mtbo_events.json`, the `sources` object provides a count and the last update timestamp for each source backend (IOF, SWE, NOR, MAN).
